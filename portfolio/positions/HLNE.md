# HLNE — Hamilton Lane

> Last reviewed: 2026-04-26 &nbsp;•&nbsp; Sector: Financials / Asset management &nbsp;•&nbsp; Target: 16% &nbsp;•&nbsp; Daily DCA: $16

## At a glance

| Metric | Value |
|---|---|
| Target allocation | 16% |
| Daily DCA | $16 |

## Thesis

Top-tier single-name pick — tied with HALO and ETN at 16% as the next-largest weights after VOO. Private-markets advisor / fund-of-funds platform riding a real secular tailwind: institutional and HNW capital continues shifting from public equities into private equity, private credit, infrastructure, and real assets. HLNE earns recurring management/advisory fees on AUM plus carry, with operating leverage as scale builds. Capital-light, founder-aligned, and trading at a fair (not stretched) multiple given AUM growth visibility.

## Bull case

- Secular shift to private markets is multi-decade — not a cycle.
- Recurring fee revenue dominates; carry is upside, not the base case.
- High insider ownership; aligned management.
- Operating leverage as platform scales — incremental margins are very high.

## Bear case & risks

- A multi-year drawdown in private markets fundraising would compress fee growth.
- Key-person concentration in an advisory business.
- Multiple compression if rates stay structurally higher and private-market returns disappoint vs. promises.
- Fee compression risk as evergreen / retail products proliferate.

## Catalysts (next 12 months)

- [ ] Quarterly AUM update with fundraising commentary
- [ ] Evergreen / retail product traction disclosure
- [ ] Earnings release — TBD by earnings-watcher

## Valuation snapshot

_Last refreshed: 2026-04-26 (placeholder)_

| Metric | This stock | Peer / history reference |
|---|---|---|
| P/E (TTM) | — | vs. asset-mgr peers |
| P/E (forward) | — | — |
| EV/EBITDA | — | — |
| FCF yield | — | — |
| AUM growth (YoY) | — | — |
| Notes | — | — |

## Capital return

- **Dividend**: quarterly, with a track record of regular increases.
- **Buybacks**: opportunistic.

## Recent earnings (last 4 quarters)

| Quarter | Revenue | EPS | Guidance change | Notes |
|---|---|---|---|---|
| — | — | — | — | — |

## News & notes

<!-- news-start -->
<!-- news-end -->

## Re-check schedule

- Monthly thesis review issue: _link once opened_
- Next earnings: _populated by earnings-watcher_

---

_Personal journal entry. Not financial advice._
